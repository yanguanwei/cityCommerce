...
import city.commerce.entity.IndexStore;
...
public class LuceneConfiguration {
    ...
    protected void createIndex(List<IndexStore> indexStores) throws Exception {
        IndexWriterConfig config = new IndexWriterConfig(new StandardAnalyzer());
        IndexWriter indexWriter = new IndexWriter(directory, config);
        indexWriter.addDocuments(newSampleDocument(ctx, strategy, indexStores));
        indexWriter.close();
    }
     ...
     * 创建Document索引对象
     */
    protected List<Document> newSampleDocument(SpatialContext ctx, SpatialStrategy strategy, List<IndexStore> indexStores) {
        List<Document> documents = Lists.newLinkedList(indexStores.stream()
                .map(cgi -> {
                    Document doc = new Document();
                    doc.add(new StoredField("id", cgi.getStoreId()));
                    doc.add(new NumericDocValuesField("id", cgi.getStoreId()));
                    doc.add(new StringField("name", cgi.getName(), Field.Store.YES));
                    Shape shape = null;
                    /**
                    ...
                    Field []fields = strategy.createIndexableFields((shape = ctx.getShapeFactory()
                            .pointXY(cgi.getLongitude(), cgi.getLatitude())));
    ...
    protected void printDocument(TopDocs topDocs, IndexSearcher indexSearcher, Point point) throws Exception {
        ...
        for (ScoreDoc scoreDoc : scoreDocs) {
            ...
            String city = document.getField("name").stringValue();
    ...
    public static void main(String[] args) throws Exception {
        int id=0;
        List<IndexStore> indexStores = new ArrayList<>();
        for (String s : Files.readAllLines(Paths.get("/Users/yan/cities.txt"))) {
            String[] a = s.split(":");
            IndexStore indexStore = new IndexStore();
            indexStore.setStoreId(id++);
            indexStore.setName(a[0].trim());
            String[] a1 = a[1].split(",");
            indexStore.setLongitude(Double.parseDouble(a1[0].trim()));
            indexStore.setLatitude(Double.parseDouble(a1[1].trim()));
            indexStores.add(indexStore);
        }

        LuceneConfiguration luceneSpatial = new LuceneConfiguration();
        luceneSpatial.init();
        luceneSpatial.createIndex(indexStores);
